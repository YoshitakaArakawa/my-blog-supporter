...
- [ ] **作図する**：`output/{記事}/images/{図名}.svg`（SVG）または `{図名}.html`（HTML medium）に保存。HTML は**フラグメント**で書く——ルート要素に `id="fig"`、`font-family:"Zen Kaku Gothic New"`、レイアウト用 `<style>` 同梱。
...
- **必ずファイルとして書き出す**。inline で図やコードを返さない。
  - `images/{図名}.svg` / `.html` … ソース（gitignore 済み。ローカルに残り、後で修正・再描画できる）
  - `images/{図名}.png`（複数案は `variant_{kk}.png`）… 成果物（gitignore 済み。Wix へ手動アップロード前提）
